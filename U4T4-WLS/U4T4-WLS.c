...
// Definição do número de frames da "animação"
 ...
 * Considera um mapeamento tipo zigue-zague.
 */
int getIndex(int x, int y) {
    if (y % 2 == 0) {
        return (y * 5) + x;  // Linhas pares: esquerda para direita
    } else {
        return (y * 5) + (4 - x);  // Linhas ímpares: direita para esquerda
    }
}
...
void display_number(uint8_t number) {
    // Definição dos números na matriz de LEDs (0 a 9)
    const int numbers_animation[ANIMATION_FRAMES][5][5][3] =
{
    {
    ...
    for (int y = 0; y < 5; y++) {
        for (int x = 0; x < 5; x++) {
            ...
            leds[posicao].r = numbers_animation[number][y][x][0];
            leds[posicao].g = numbers_animation[number][y][x][1];
            leds[posicao].b = numbers_animation[number][y][x][2];
        }
    }
